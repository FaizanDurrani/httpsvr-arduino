...
public:
  ...
  public:
    ...
    // By default, it reads the SIPR (Source IP Address), but it can be used
    // for any other IP address register. For example, passing Sn_DIPR0
    // as parameter, it reads the Socket n's Destination IP Address.
    // Also note that the first form is understood as the default constructor!
    ...
    ipv4_address_t(const uint8_t * the_ip);
    ipv4_address_t(const IPAddress& the_ip);
        
  public:
    // This method writes the IP address to W5100 registers.
    // By default, it writes the SIPR (Source IP Address), but it can be used
    // for any other IP address register. For example, passing Sn_DIPR0
    // as parameter, it writes the Socket n's Destination IP Address
    void              set(uint16_t the_regAddr = W5100_SIPR0) const;

    // These methods return each single component of the IP address
    // stored in the member variable
    inline uint8_t    ip0 () const { return my_addr[0]; }
    inline uint8_t    ip1 () const { return my_addr[1]; }
    inline uint8_t    ip2 () const { return my_addr[2]; }
    inline uint8_t    ip3 () const { return my_addr[3]; }
    
    // Make and Test functions for null values, i.e. 0.0.0.0 address
    static ipv4_address_t makeNull  ()       { return ipv4_address_t(0,0,0,0); }
    bool                  isNull    () const { return (ip0() | ip1() | ip2() | ip3()) == 0; }
    
    // This static function buids an IPV4 from a sequence of chars.
    // It is useful e.g. for translating DNS response data into a real IP
    static ipv4_address_t fromString(const char * the_s);
...
public:
  ...
  static void         begin               (const mac_address_t&  the_macAddr,
                                           const ipv4_address_t& the_srcIpAddr,
                                           const ipv4_address_t& the_subnetMask = ipv4_address_t(0,0,0,0),
                                           const ipv4_address_t& the_gatewayIpAddr = ipv4_address_t(0,0,0,0));
                                           
  static void         terminate           ();

public:
  // Socket command functions
  static retcode_e    openTCP             (socket_e the_socket, uint16_t the_srcPort);
  static retcode_e    openUDP             (socket_e the_socket, uint16_t the_srcPort);
  ...
  static uint16_t     send                (socket_e the_socket, const ipv4_address_t& the_ipAddr, uint16_t the_dstPort, uint8_t * the_buffer, uint16_t the_size);
  ...
  static uint8_t      mode                (socket_e the_socket);
  ...
  static bool         isTCP               (socket_e the_socket);
  static bool         isUDP               (socket_e the_socket);
