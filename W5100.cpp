...
void W5100::begin(const mac_address_t&  the_macAddr,
                  const ipv4_address_t& the_srcIpAddr,
                  const ipv4_address_t& the_subnetMask,
                  const ipv4_address_t& the_gatewayIpAddr)
{
  ...
  // then subnet mask and gateway address if specified.
  the_macAddr.set();
  the_srcIpAddr.set();
  if (!the_subnetMask.isNull()) the_subnetMask.set(W5100_SUBR0);
  if (!the_gatewayIpAddr.isNull()) the_gatewayIpAddr.set(W5100_GAR0);
}                                           
...
W5100::retcode_e W5100::openTCP(socket_e the_socket, uint16_t the_port)
{
...
W5100::retcode_e W5100::openUDP(socket_e the_socket, uint16_t the_srcPort)
{
  // Check preconditions: socket status must be CLOSED or UDP
  uint8_t sockStatus = status(the_socket);
  if ((sockStatus != W5100_SOCK_UDP) && (sockStatus != W5100_SOCK_CLOSED))
    return rc_invalid_status;
  
  // Clear any previous event flag
  write_Sn_R8(the_socket, W5100_Sn_IR, 0xFF);
  
  // Set socket mode (UDP) and local port
  write_Sn_R8 (the_socket, W5100_Sn_MR, W5100_PROTOCOL_UDP);
  write_Sn_R16(the_socket, W5100_Sn_PORT, the_srcPort);
  
  // Issue the OPEN command and wait for completion
  for (unsigned int uTry = 0; uTry < uMaxTry; ++uTry)
  {
    write_Sn_R8(the_socket, W5100_Sn_CR, W5100_COMMAND_OPEN);
    if (read_Sn_R8(the_socket, W5100_Sn_SR) == W5100_SOCK_UDP) return rc_ok;
  }
  return rc_open_failed;
}
...
{
  // Check preconditions: socket status must be ESTABLISHED for TCP protocol...
  if (isTCP(the_socket) && (status(the_socket) != W5100_SOCK_ESTABLISHED))
    return 0;
  
  // ...or UDP for UDP protocol.
  if (isUDP(the_socket) && (status(the_socket) != W5100_SOCK_UDP))
    return 0;

  return prv_txData(the_socket, the_buffer, the_size);
 }

uint16_t W5100::send(socket_e the_socket, const ipv4_address_t& the_ipAddr, uint16_t the_dstPort, uint8_t * the_buffer, uint16_t the_size)
{
  // Set IP address and port number to Sn_DIPR and Sn_DPORT respectively
  switch (the_socket)
  {
  case socket_0: the_ipAddr.set(W5100_S0_DIPR0); break;
  case socket_1: the_ipAddr.set(W5100_S1_DIPR0); break;
  ...
  write_Sn_R16(the_socket, W5100_Sn_DPORT, the_dstPort);

  return send(the_socket, the_buffer, the_size); 
}
...
uint8_t W5100::mode(socket_e the_socket)
{ return read_Sn_R8(the_socket, W5100_Sn_MR); }
...
bool W5100::isTCP(socket_e the_socket)
{ return (mode(the_socket) & W5100_PROTOCOL_MASK) == W5100_PROTOCOL_TCP; }

////////////////////////////////////////////////////////////////////////////////

bool W5100::isUDP(socket_e the_socket)
{ return (mode(the_socket) & W5100_PROTOCOL_MASK) == W5100_PROTOCOL_UDP; }
...
W5100::ipv4_address_t::ipv4_address_t(const IPAddress& the_ip)
{
  my_addr[0] = the_ip[0];
  my_addr[1] = the_ip[1];
  my_addr[2] = the_ip[2];
...
static const char * local_toByte(const char *the_s, uint8_t& the_byte)
{
  the_byte = 0;
  uint16_t u = 0;
  const char *s = the_s;
  ...
  the_byte = static_cast<uint8_t>(u);
  return s;
}

W5100::ipv4_address_t W5100::ipv4_address_t::fromString(const char * the_s)
{
  // The string is translated into an IP address as follows:
  // "192.168.0.1" => addr[0] = 192; addr[1] = 168; addr[2] = 0; addr[3] = 1;
  // This should be consistent to W5100 byte order in SIPR, GAR, SUBR and DIPR,
  // and the behavior of function "set".

  // Check preconditions
  if (!the_s) return makeNull();
  if (strlen(the_s) < 15) return makeNull();

  // Assign values  
  const char *s = the_s;
  ipv4_address_t ipv4 = makeNull();
  s = local_toByte(  s, ipv4.my_addr[0]); if (!s) return makeNull();
  s = local_toByte(++s, ipv4.my_addr[1]); if (!s) return makeNull();
  s = local_toByte(++s, ipv4.my_addr[2]); if (!s) return makeNull();
  s = local_toByte(++s, ipv4.my_addr[3]); if (!s) return makeNull();
